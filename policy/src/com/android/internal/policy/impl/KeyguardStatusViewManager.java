    ...
    private CharSequence getAltTextMessage(MutableInt icon) {
        ...
        if (mShowingBatteryInfo || mLockAlwaysBattery) {
            ...
            if (mPluggedIn) {
                ...
                //FIXME SunRain
                // mLockAlwaysBattery should == mAlwaysShowBattery 
                // as mLockAlwaysBattery for aokp and mAlwaysShowBattery for cm
                // need to clean up later
                if (mLockAlwaysBattery || mAlwaysShowBattery) {
                    // Show battery at low percent
                    string = getContext().getString(R.string.lockscreen_always_low_battery, mBatteryLevel);
                            icon.value = BATTERY_LOW_ICON;
                }                
            } else if (mAlwaysShowBattery || mLockAlwaysBattery) {
                // Discharging
                // Always show battery
